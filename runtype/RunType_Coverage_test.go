...
// AL2-04 Batch D coverage suite for runtype.

var allRunNames = []string{"Now", "OnReboot", "OnShutdown", "OnEveryReboot",
	"OnEveryShutdown", "OnFailRetry", "EveryMinute", "EveryHour", "EveryDay",
	"EveryMonth", "EveryYear"}

func TestRunType_NewAndAccessors(t *testing.T) {
	for _, name := range allRunNames {
		v, err := New(name)
		if err != nil {
	...
	if !v.IsByteValueEqual(byte(Now)) || v.IsByteValueEqual(byte(OnReboot)) ||
		!v.IsValueEqual(byte(Now)) || !v.IsNameEqual("Now") {
		t.Error("equality wrong")
	}
	if !v.IsAnyValuesEqual(byte(Now), byte(OnReboot)) ||
		!v.IsAnyNamesOf("Now", "OnReboot") {
		t.Error("IsAny wrong")
	}
	...
	_ = v.OnlySupportedErr("Now")
	_ = v.OnlySupportedMsgErr("ctx", "Now")
}
