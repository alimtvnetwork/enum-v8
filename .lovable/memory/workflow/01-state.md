...
> **Last updated:** 2026-05-06 (Cycle 32 — S-110 RESOLVED: built the 3 standalone helpers in `scripts/coverage/` (`Get-UncoveredLines.ps1`, `Get-FunctionCoverage.ps1`, `Get-PackageCoverageReport.ps1` with `-Format text|markdown|json`). All smoke-tested via nix-pwsh against synthetic inputs. D-CVS-62 now fully closed — `spec/03-powershell-test-run/06-coverage-prompt-generator.md` and `scripts/coverage/` are in lockstep).
