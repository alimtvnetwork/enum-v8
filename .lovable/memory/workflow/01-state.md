...
> **Last updated:** 2026-05-06 (Cycle 33 — S-107 RESOLVED: added `Get-GoconveyFailureSummary` to `scripts/TestLogWriter.psm1`; `failing-tests.txt` now leads each failed-test block with `── Failure summary (N) ──` listing every Expected/Actual/(Line)/[Message] triplet BEFORE the noisy raw block. Smoke-tested via nix-pwsh; both real and edge-case inputs pass.).
