...
## Core
This is a Go enum library (enum-v4, module path `github.com/alimtvnetwork/enum-v4` — renamed from `enum-v2`, originally `enum-v1`). The frontend React/Vite shell is incidental — work happens in Go packages and PowerShell/Python tooling.
...
The `cross-repo/core-v8/` directory mirrors CI surface to a separate upstream repo and intentionally keeps its `core-v8` name. Do not rename it. Also do not rewrite `enum-v1` → `enum-v4` inside this directory — it tracks a different module.
