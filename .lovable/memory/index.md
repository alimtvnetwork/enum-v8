...
The `cross-repo/core-v8/` directory mirrors CI surface to a separate upstream repo and intentionally keeps its `core-v8` name. Do not rename it. As of 2026-05-06, `enum-v3` → `enum-v7` rename WAS applied inside `cross-repo/` per user direction (one-time exception to the usual leave-cross-repo-alone rule).
