...
`enum-v2` source imports `github.com/alimtvnetwork/core-v9/...` but the upstream `core-v9` repository's own `go.mod` declares `module github.com/alimtvnetwork/core-v8`. A `replace` bridge in `enum-v2/go.mod` redirects resolution to the cached `core-v8` artifact, but Go 1.25 rejects this with `used for two different module paths`, and Go's `internal/` rule rejects any `enum-v2` consumer reaching into `core-v9/.../internal/...` because it sees the cached path as `core-v8`.
...
- [ ] Pin Go toolchain to 1.22 in `enum-v2/go.mod` — offered as stopgap, user has not accepted.
