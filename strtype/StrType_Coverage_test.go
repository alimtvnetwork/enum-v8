...
func Test_StrType_Coverage(t *testing.T) {
	...
	Convey("strtype — string ops", t, func() {
		...
		So(Variant("hello").AddSuffixOnMissing("-x").Value(), ShouldEqual, "hello-x")
		So(Variant("hello-x").AddSuffixOnMissing("-x").Value(), ShouldEqual, "hello-x")
		So(Variant("hello").AddPrefixOnMissing("p-").Value(), ShouldEqual, "p-hello")
		So(Variant("p-hello").AddPrefixOnMissing("p-").Value(), ShouldEqual, "p-hello")
