...
### Changed
- Repository renamed from `enum-v6` to `enum-v8` per upstream rename. Bulk substitution applied across `go.mod`, Go source imports, all `spec/`, `.lovable/` memory files, workflows, and `cross-repo/core-v8/README.md` (one-time exception). `.release/`, `.git/`, and the `cross-repo/core-v8/` directory name preserved.
