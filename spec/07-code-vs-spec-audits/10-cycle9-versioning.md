...
Results:
- **Zero importers** of `coreversion` or `versionindexes` in `enum-v6`.
...
- `.release/`, `.lovable/`, `mem/` directories **do not exist on disk** in `enum-v6` — citations in §3, §5, §11 source-attribution are stale.
...
| 9  | §3 (citation) | Rule sourced from `.lovable/user-preferences` line 8 | ⚠️ | **Drift D-CVS-31** — `.lovable/` directory does not exist in `enum-v6`. Citation is stale; rule lives in `mem://index.md` Core only. |
...
| 12 | §4 line 105 | "imports from `github.com/alimtvnetwork/core-v9/<pkg>` will not break within an era" | ✅ | Verified — `enum-v6` imports use `core-v9` consistently (Cycle 1 + memory Core rule) |
...
| 17 | §6 (anti-pattern) | "Using `golang.org/x/mod/semver` directly" — Common Mistake | ✅ | Verified — `rg "golang.org/x/mod/semver"` → 0 hits in `enum-v6` |
...
`.lovable/` does not exist in `enum-v6`. Both rules (`bump at least minor`, `never touch .release/`) live in `mem://index.md` Core. **Fix**: rewrite citations to point only to `mem://index.md` Core, drop the `.lovable/user-preferences line 8` reference.
