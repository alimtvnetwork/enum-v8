...
**Method:** Manual claim extraction from spec; automated `rg` verification of consumer-side usage in `enum-v3`. Upstream `core-v9` source (where `errcore` is defined) is **not** checked into this repo and the sandbox has no Go toolchain, so the audit is **consumer-anchored**: a claim is "match" only if `enum-v3` actually exercises the named symbol in the documented shape.
...
`errcore` lives in `github.com/alimtvnetwork/core-v9` (declared module name: `core-v9`). The spec describes that upstream package, but `enum-v3` is the only artifact we can read. We can therefore verify:

1. **Symbols `enum-v3` calls** — direct evidence the documented API exists and is used.
2. **Symbols `enum-v3` calls but the spec does NOT document** — drift: spec is incomplete relative to the API consumers actually rely on.
3. **Symbols the spec documents but `enum-v3` never calls** — *unverifiable* from this repo (could exist upstream and just be unused here, or could be aspirational). Marked **❓** rather than ❌.
...
## Symbols actually used by `enum-v3` (`rg -o 'errcore\.\w+(\.\w+)?' -g '*.go'`)
...
| C18 | Decision tree: unparseable shape → `FailedToConvertType`; parses but rule-rejects → `ValidationFailedType` (§"Boundary Cases") | Pattern is *consistent* with how `enum-v3`'s `all-validation-checking-err.go` files segregate parse failure from range failure, though they call `RangeNotMeet` / `ValidationError` (helpers) rather than the raw type. No contradictory usage found. |

### ⚠️ Drift — spec is incomplete (8)

These are the highest-impact findings: `enum-v3` depends on APIs the spec does not mention. A new contributor reading §04 would not learn the patterns the codebase actually uses.
...
| D-CVS-13 | Several `RawErrorType` values used in `enum-v3` are not in the §1.1 "Common categories" list: `FailedToExecuteType`, `NotSupportedType`, `PathInvalidErrorType`, `ComparatorShouldBeWithinRangeType` | Direct call sites listed above | LOW | Either expand the §1.1 examples or add a footnote pointing to the upstream `RawErrorType.go` enumeration |

### ❓ Unverifiable from this repo (7)

Spec lists these APIs but no `enum-v3` source calls them. They may exist upstream — pending the upstream-source audit (task **AB**), they cannot be confirmed or refuted here.

| # | Claim | Why unverifiable |
|---|-------|------------------|
| C1 | `RawErrorType` exposes 80+ predefined values (§1.1) | Need upstream `errcore/RawErrorType.go` to count |
| C2 | `Error(name, ref)` constructor exists (§1.2) | No call site in `enum-v3` (uses `ErrorRefOnly` / `Error(...)` on specific types instead) |
| C3 | `ErrorNoRefs`, `Fmt`, `FmtIf`, `MergeError`, `MergeErrorWithMessage` constructors (§1.2 table) | Zero call sites in `enum-v3` |
...
> ⚠️ The fact that **none** of the §3, §6, and most of §1.2 are exercised by `enum-v3` is itself a meta-finding: either the spec describes a much wider API than this repo needs (legitimate — `core-v9` serves many consumers), OR the spec drifted away from current upstream. Resolving this requires fetching `core-v9` source (task **AB**).
...
Excluding the 7 ❓ from the denominator avoids penalising the spec for documenting upstream APIs unused by `enum-v3`. Once task **AB** brings in `core-v9` source, the 7 ❓ will be resolved into match/drift/contradiction and the score recomputed.
