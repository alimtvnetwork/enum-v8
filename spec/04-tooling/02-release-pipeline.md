...
Library release pipeline for `enum-v5`. Triggers on `release/**` branches and `v*` tags. Produces source archives + checksums + a GitHub Release with an extracted changelog section.
...
| Package source archives | `dist/enum-v5-<ver>-source.tar.gz`, `.zip`, `checksums.txt` (SHA256) |
...
| Module  | `github.com/alimtvnetwork/enum-v5` |

### Install
```bash
go get github.com/alimtvnetwork/enum-v5@vX.Y.Z
```
