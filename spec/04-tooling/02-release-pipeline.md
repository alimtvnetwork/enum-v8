...
Library release pipeline for `enum-v3`. Triggers on `release/**` branches and `v*` tags. Produces source archives + checksums + a GitHub Release with an extracted changelog section.
...
| Package source archives | `dist/enum-v3-<ver>-source.tar.gz`, `.zip`, `checksums.txt` (SHA256) |
...
### Install
```bash
go get github.com/alimtvnetwork/enum-v3@vX.Y.Z
```
