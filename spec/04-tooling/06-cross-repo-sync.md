...
`enum-v5` depends on `core-v9` and shares the same CI/CD philosophy with
...
3. Add a comment to the target-repo workflow header pointing back at
   this spec, e.g.:
   ```yaml
   # Synced from github.com/alimtvnetwork/enum-v5/cross-repo/core-v8/
...
- **A reusable workflow** in a shared `alimtvnetwork/.github` repo, with
  both `enum-v5` and `core-v9` calling it via `uses:`.
