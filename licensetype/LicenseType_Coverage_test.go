...
// AL2-05 Batch E coverage suite for licensetype.

func TestLicenseType_NewAndAccessors(t *testing.T) {
	for _, name := range []string{"PublicDomain", "ByCc", "BySa", "ByNc", "ByNcSa", "ByNd", "ByNcNd"} {
		v, err := New(name)
		if err != nil {
	...
	_ = v.OnlySupportedErr("ByCc")
	_ = v.OnlySupportedMsgErr("ctx", "ByCc")
}
